...
export interface GetProps<TData, TError, TQueryParams> {
  ...
  /**
   * A key used to set/access the cache for this component.
   *
   * Overrides the default cache key that is generated
   *
...
> {
  ...
  public fetch = async (requestPath?: string, thisRequestOptions?: RequestInit, ignoreCache?: boolean) => {
    ...
    // Create a cache key for easier cache lookup
    const internalCacheKey =
      cacheKey ||
      toStr({
  ...
  public render() {
    ...
    return children(
      ...
      { loading, error },
      {
        refetch: (): Promise<any> => this.fetch(undefined, undefined, true),
      },
